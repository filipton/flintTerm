...
private fun StatusWidgetConfig(widgetId: Int, onPick: (hostId: String, minutes: Int) -> Unit) {
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState())) {
        AppHeader("Server status", subtitle = "Load, memory and the fullest disk, on your home screen")

        Group(title = "Check every") {
        ...
        if (sorted.isEmpty()) {
            EmptyState(
                Icons.Rounded.Dns,
                "No hosts yet",
                "Add a host in flintTerm and this widget can watch it.",
            )
        } else {
            Group(title = "Watch") {
